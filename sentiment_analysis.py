...
def load_sentiment_model():
    import transformers
    # Using your specified base model (Update this path to your locally fine-tuned model path when ready!)
    model_path = "cardiffnlp/twitter-xlm-roberta-base-sentiment"
    
    # top_k=None forces the pipeline to return scores for ALL labels, not just the top 1
...
<div class="premium-header">
    <div>
        <h1>🧠 Multilingual Sentiment Engine</h1>
        <p>Test the fine-tuned XLM-RoBERTa model on English, Tagalog, and Taglish PUV commuter feedback.</p>
    </div>
</div>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════
# TABS: Playground vs Batch
# ══════════════════════════════════════════
tab_single, tab_batch = st.tabs(["🧪 Single Analysis Playground", "📁 Batch CSV Processing"])

with tab_single:
    st.write("**Enter Commuter Feedback:**")
    user_input = st.text_area("", placeholder="Example: Sobrang init sa loob ng jeep at ang sungit ng driver, never again! 😠", height=120, label_visibility="collapsed")
    
    if st.button("🚀 Analyze Sentiment"):
        if user_input.strip():
            # Run prediction (returns a list of dictionaries for all classes)
            results = classifier(user_input)[0] 
            
            # Re-format the results into a clean dictionary: {"Positive": 0.85, "Neutral": 0.10, "Negative": 0.05}
            scores_dict = {label_map.get(res['label'], str(res['label']).capitalize()): res['score'] for res in results}     

            # Identify the winning sentiment
            top_sentiment = max(scores_dict, key=scores_dict.get)
            top_score = scores_dict[top_sentiment]
            
            st.markdown("<div style='height: 20px;'></div>", unsafe_allow_html=True)
            
            # UI Layout: Left side Winner, Right side Distribution
            ...
            with col2:
                ...
                # Draw custom distribution bars for all three classes
...
with tab_batch:
    st.info("Upload a CSV file containing a column named **'feedback'** to analyze multiple responses at once.")
