...
# ── 3. Global CSS ──
# (Kept your CSS exactly as provided)
st.markdown("""
<style>
...
# ── 4. SECURE SESSION RESOLUTION (Cross-Device Isolation Fix) ──
if "logged_in" not in st.session_state:
    st.session_state.logged_in = False

try:
    # PRIORITY 1: The URL "Session" Token
    # This ensures that if you just logged in on THIS device, we use that data.
    if "session" in st.query_params:
        ...
        # We clear it immediately so the URL can't be shared/refreshed into a different account
        st.query_params.clear()

    # PRIORITY 2: Persistent Supabase Check (ONLY if Session State is not already set)
    elif not st.session_state.logged_in:
        # We check the actual Supabase client. 
        # Note: In cross-device scenarios, this is where "ghost" logins usually happen.
        ...
        if user_res and user_res.user:
            ...
            st.session_state.first_name = metadata.get("first_name", metadata.get("full_name", "Admin"))
...
if "form_id" in st.query_params:
    ...
    with st.sidebar:
        ...
        # ── MODIFIED LOGOUT: The "Nuke" Option ──
        if st.button("🚪 Hard Logout", use_container_width=True):
            try:
                # 1. Tell Supabase to invalidate all sessions for this user globally
                conn.client.auth.sign_out(scope='global')
            except: pass
            
            # 2. Clear server-side state
            st.query_params.clear()
            st.session_state.clear()
            
            # 3. Force the browser to clear localStorage/sessionStorage via JS
            # This prevents the "auto-login" on next page load.
            components.html("""
                <script>
