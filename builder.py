...
with col3:
    st.write("") 
    st.write("") 
    is_demographic = st.checkbox("👥 Demographic?")
...
if st.button("➕ Add Question"):
    if new_prompt:
        new_q = {
            ...
            "public_id": public_id # <--- We save the ID to the database here!
        }
...
for i, q in enumerate(form_schema):
    ...
    demo_badge = " *(👥 Demographic)*" if q.get('is_demographic') else ""
    c1.write(f"**{i+1}. {q['prompt']}** *(Type: {q['q_type']})*{demo_badge}")
    
    if c2.button("❌ Delete", key=f"del_{q['id']}"):
